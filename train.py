...
class Trainer():
    ...
    def train(self, epoch):
        ...
        self.criterion.to(dtype=self.dtype)
    ...
    def evaluate(self, epoch, mode='val', test_step=70):
        self.mode = mode
        self.epoch = epoch
        ...
        if mode == 'test':
            if self.rotational:
                self.criterion.set_teststep(test_step)
                self.loaders['test'].dataset.set_teststep(test_step)
    ...
    def test(self, epoch, test_step=70):
        self.evaluate(epoch, 'test', test_step=test_step)
        return

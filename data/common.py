...
def chose_part(*args, ps=256):    # patch_size
    # args = [input, target]
    def _get_shape(*args):
...
def crop(*args):
    ...
    def _crop(img):
        if img.ndim == 2:
            return img[start:start+height, start:start+height]
        else:
            return img[start:start + height, start:start + height, :]
...
def add_noise(*args, mean=0, rgb_range=255, std=0.01):
    ...
    def _add_noise(image):
        ...
        return (image + noisy).clip(0, rgb_range)
...
def augment_for_real(*args, rgb_range=255):
    ...
    methods = (saturation, shuffle_img)
...
def np2tensor_evl(*args):
    def _np2tensor(x):
        np_transpose = np.ascontiguousarray(x.transpose(2, 0, 1))
        tensor = torch.from_numpy(np_transpose)
        tensor = tensor.unsqueeze(0).type(torch.FloatTensor).cuda()
        return tensor
    return _apply(_np2tensor, args)
